...
const EditStationSection: React.FC<Props> = ({ messageManagerRef }) => {
  ...
  const onCopyStationForwards = () => {
    postMessageToPlugin({ type: 'copy-station', stationId: stationId!, direction: 'forwards' });
  };

  const onCopyStationBackwards = () => {
    postMessageToPlugin({ type: 'copy-station', stationId: stationId!, direction: 'backwards' });
  };
  ...
  const onStartCombineMode = () => setIsCombiningMode(true);
  const onCancelCombineMode = () => setIsCombiningMode(false);
  ...
  useEffect(() => {
    ...
    if (name !== stationName) {
      nameUpdateTimerRef.current = setTimeout(() => {
        onUpdateStation(name, textAlign);
      }, 500);
    }
